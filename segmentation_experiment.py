...
def get_experiment_preprocess_configs():
    ...
    smoothing_gaussian_values = [1.0, 2.0, 5.0]
    smoothing_median_values = [1, 2, 53]
    morphology_radius = [2, 3, 4, 5]

    smoothing_options = list(itertools.product([True], ["gaussian"], smoothing_gaussian_values))
    smoothing_options.append(list(itertools.product([True], ["median"], smoothing_median_values)))
...
if __name__ == "__main__":
    experiment_name = "experiments_seg"
    experiment_root = os.path.join(PROJECT_DIR, experiment_name)
    ...
    for idx, prepro_config in enumerate(prepro_configs):
        ...
        idxs = (idx * len(train_configs) + 1, idx * len(train_configs) + 1 + len(train_configs))
